...
import { saveWeddingReferences } from './acitons';
...
describe('saveWeddingReferences', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should return error if photos array is empty', async () => {
    const result = await saveWeddingReferences([]);
  ...
  it('should successfully save photos and revalidate path', async () => {
    ...
    const result = await saveWeddingReferences(photos);
